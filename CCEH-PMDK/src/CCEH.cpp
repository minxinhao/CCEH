...
void CCEH::initCCEH(PMEMobjpool* pop){
    ...
    struct Segment** temp = (struct Segment**)malloc(sizeof(TOID(struct Segment))*D_RO(dir)->capacity);
    TOID(struct Segment)* temp2 = (TOID(struct Segment)*)temp; 
...
DIR_RETRY:
	...
	struct Directory* _dir = (struct Directory*)malloc(sizeof(struct Directory));
        _dir->segment = (TOID_ARRAY(TOID(struct Segment)))malloc(sizeof(TOID(struct Segment))*mem_dir->capacity);
